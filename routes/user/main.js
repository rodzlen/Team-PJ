      ...
      db.query(query, values, (err, result) => {
        if (err) {
          ...
          res
            .status(500)
            .send(
              '<script>alert("공지사항 추가 중 오류가 발생했습니다."); window.location.href="/user/freeboard/add";</script>'
            );
        } else {
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    let query =
      "SELECT q.id AS question_id, q.question, q.question_date, q.asked_by,  a.id AS answer_id, a.answer, a.answer_date, a.answered_by  FROM Questions q  LEFT JOIN Answers a ON q.id = a.question_id";
    let queryParams = [];
    search(query, searchQuery, typeQuery);
...
router.post("/edit/:id", async (req, res) => {
  ...
  try {
    await db.query("UPDATE Questions SET title = ?, content = ? WHERE id = ?", [
      title,
      content,
      questionId,
    ]);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (current_password !== confirm_password) {
      return res
        .status(400)
        .json({ error: "기존 비밀번호와 비밀번호 확인이 일치하지 않습니다." });
    }
